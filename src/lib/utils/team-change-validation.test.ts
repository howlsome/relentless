...
import { validateTeamChange, checkDesignationParity } from './team-change-validation.js';
import type { Roster } from '$lib/types/roster.js';

function makeRoster(players: any[] = []): Roster {
	return {
		app_name: 'Test', realm: 'D', region: 'eu',
		mplus_weekly_minimum: 4, mplus_minimum_key_level: 10,
		tracking_start_date: '2026-01-01',
		mplus_seasons: [], raid_difficulties: [], wcl_expansion_id: 11,
		players
	} as Roster;
}
...
describe('validateTeamChange', () => {
	it('returns error when reason is missing on team_changed event', () => {
		const event = { event: 'team_changed' as const, date: '2026-03-17', from: 'alt' as const, to: 'main' as const, reason: '' };
	...
	it('returns valid when reason is non-empty', () => {
		const event = { event: 'team_changed' as const, date: '2026-03-17', from: 'alt' as const, to: 'main' as const, reason: 'Strong performance' };
