...
const activeChar = {
	name: 'Howlsome', realm: 'Draenor', class: 'DeathKnight', spec: 'Unholy', role: 'dps', active: true, first_seen: '2026-03-17'
};
const inactiveChar = {
	name: 'Voidclaw', realm: 'Draenor', class: 'DemonHunter', spec: 'Havoc', role: 'dps', active: false, first_seen: '2026-01-01', last_seen: '2026-03-17'
};
...
describe('CharacterParseSection', () => {
	it('active character section has open attribute (expanded on load)', () => {
		const { container } = render(CharacterParseSection, {
			character: activeChar, parses: [], difficulty: 'mythic', isActive: true
		});
	...
	it('inactive character section does NOT have open attribute (collapsed on load)', () => {
		const { container } = render(CharacterParseSection, {
			character: inactiveChar, parses: [], difficulty: 'mythic', isActive: false,
			dateRange: '1 Jan 2026 → 17 Mar 2026', bestParseSummary: 'Best: 85%'
		});
		const details = container.querySelector('details');
	...
	it('inactive section header shows best parse summary', () => {
		const { container } = render(CharacterParseSection, {
			character: inactiveChar, parses: [], difficulty: 'mythic', isActive: false,
			dateRange: '1 Jan → 17 Mar', bestParseSummary: 'Best: 85%'
		});
		const summary = container.querySelector('summary');
