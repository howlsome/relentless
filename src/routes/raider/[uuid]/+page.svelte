<script lang="ts">
	...
	let {
		...
		data: {
			...
			primaryRaidZone: {
				...
				raiderData: RaidRaiderEntry | null;
				season_id: string;
			} | null;
	...
	const _dungeons = $derived(activeSeason?.dungeons ?? []);
...
{:else}
	...
	<header class="raider-header">
		...
		<div class="raider-header__badges">
			...
			{#if isOnTrack === true}
				<span class="status-badge status-badge--met" aria-label="M+ requirement met this week"
					>On track</span
				>
			{:else if isOnTrack === false}
				<span
					class="status-badge status-badge--behind"
					aria-label="M+ requirement not yet met — still time this week">Behind this week</span
				>
			{:else}
			...
			{#if lockoutRaider?.lockout_warnings?.length}
				<span
					class="status-badge status-badge--blocking"
					aria-label="Progression-blocking pug this week">🚨 Progression-blocking pug</span
				>
			{/if}
			{#if lockoutRaider?.safe_pug_kills?.length}
				<span class="status-badge status-badge--safe-pug" aria-label="Safe pugs this week"
					>🌱 Progress pugging</span
				>
			{/if}
		</div>
	</header>
	...
	{#if lockoutRaider?.lockout_warnings?.length}
		<div class="blocking-warn" role="alert" aria-label="Progression-blocking pug warnings">
			<p class="blocking-warn__title">
				🚨 Progression-blocking pug{lockoutRaider.lockout_warnings.length > 1 ? 's' : ''} this reset — locked
				out for the entirety of this reset.
			</p>
			<ul class="blocking-warn__list">
				{#each lockoutRaider.lockout_warnings as w}
					<li>
						{w.difficulty.charAt(0).toUpperCase() + w.difficulty.slice(1)} — {w.boss_name}{#if w.wcl_report_code}&nbsp;&mdash;
							<a
								href="https://www.warcraftlogs.com/reports/{w.wcl_report_code}#fight={w.wcl_fight_id ??
									'last'}"
								target="_blank"
								rel="noopener noreferrer"
								class="wcl-link">Logs</a
							>{/if}
					</li>
				{/each}
			</ul>
		...
		<div class="exempt-notes">
			{#each lockoutRaider.exempt_pug_kills as e}
				<div class="exempt-note">
					<span
						>ℹ️ Mythic <strong>{e.boss_name}</strong> pugged outside Relentless — exempt by
						<strong>{e.exemption_granted_by}</strong>{e.exemption_reason
							? `: ${e.exemption_reason}`
							: ''}.</span
					>
				</div>
	...
	{#if activeChar}
		<details class="char-wrapper" open>
			<summary class="char-wrapper__summary">
				...
				<span class="char-wrapper__detail">
					{activeChar.specs?.length
						? (getPrimarySpec(activeChar)?.spec ?? activeChar.spec)
						: activeChar.spec}
						...
						<div class="difficulty-toggle" role="group" aria-label="Select difficulty">
							{#each primaryRaidDifficulty === 'mythic' ? [['mythic', 'Mythic'], ['heroic', 'Heroic']] : [['heroic', 'Heroic'], ['mythic', 'Mythic']] as [val, label]}
								<button
									...
									aria-pressed={difficulty === val}>{label}</button
								>
							{/each}
						</div>
			...
			<details class="zone-wrapper" open>
				<summary class="zone-wrapper__summary"
					>Mythic+ — {activeSeason?.label ?? 'Current Season'}</summary
				>
	...
	<RaiderTimeline membershipHistory={raider.membership_history} roleHistory={raider.role_history} />
{/if}
...
<style>
	...
	.char-wrapper__summary::-webkit-details-marker {
		display: none;
	}
	...
	.zone-wrapper__summary::-webkit-details-marker {
		display: none;
	}
